...
- `GET /kursy` — katalog aktywnych ofert (sama kwota, bez „od” i bez „/ osoba”; niezakupione: „Zamawiam dostęp” + „Zobacz szczegóły”; sprzedaż zamknięta: wyszarzony, nieaktywny „Sprzedaż zamknięta”; 25 kursów na stronę; najpierw sprzedaż otwarta, potem wyłączone, w kolejności z ADM),
...
- `GET|POST /kursy/{product:slug}/zamowienie` — checkout płatnych wariantów w 4 krokach jak szkolenie V2 (profil → kontakt → faktura → płatność); nowy zakup startuje od „Osoba prywatna” i płatności online, gdy bramka jest dostępna; przy osobie prywatnej przełącznik „Zamawiający jest równocześnie uczestnikiem kursu” kopiuje imię, nazwisko i e-mail do uczestnika oraz nabywcy na fakturze; Imię, Nazwisko, e-mail i telefon kontaktu są w jednym wierszu jak na szkoleniu, a telefon zamawiającego jest wymagany; jeden wariant jest w podsumowaniu, nie jako osobny krok,
...
Katalog pokazuje kursy z publiczną ofertą. **Można kupić** i **pokazuj w katalogu** są niezależne: kurs może być w `/kursy` z wyszarzonym, nieaktywnym przyciskiem „Sprzedaż zamknięta” (portfolio / nieaktualna wersja). Taka strona oferty ma `noindex` i nie trafia do sitemapy. Checkout jest zablokowany. Osoba z aktywnym dostępem nadal widzi „Przejdź do kursu”. Kolejność na `/kursy` ustawiasz w ADM na `/online-courses` (przeciąganie / strzałki). Lista publiczna najpierw pokazuje kursy ze sprzedażą, potem wyłączone.
